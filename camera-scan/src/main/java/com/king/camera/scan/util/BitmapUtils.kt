...
object BitmapUtils {
    ...
    fun getBitmap(nv21Data: ByteArray, frameMetadata: FrameMetadata): Bitmap? {
        return getBitmap(ByteBuffer.wrap(nv21Data), frameMetadata.width, frameMetadata.height, frameMetadata.rotation)
    }
    ...
    fun getBitmap(data: ByteBuffer, width: Int, height: Int, rotationDegrees: Int): Bitmap? {
        ...
        return try {
            ...
            stream.close()
            rotateBitmap(bmp, rotationDegrees, false, false)
        } catch (e: Exception) {
            LogX.e(e)
            null
        }
    ...
    fun getBitmap(image: ImageProxy): Bitmap? {
        ...
        val nv21Data = yuv420ThreePlanesToNV21(image.image!!.planes, image.width, image.height)
        return getBitmap(ByteBuffer.wrap(nv21Data), image.width, image.height, image.imageInfo.rotationDegrees)
    }
    ...
    private fun getExifOrientationTag(resolver: ContentResolver, imageUri: Uri): Int {
        ...
        if (!ContentResolver.SCHEME_CONTENT.equals(imageUri.scheme)
            && !ContentResolver.SCHEME_FILE.equals(imageUri.scheme)) {
            return 0
        }

        val exif: ExifInterface
        try {
            val inputStream = resolver.openInputStream(imageUri) ?: return 0
            inputStream.use {
